...
class DsaStats:
    ...
    def talentResult(self, secondLine: str, thirdLine: str):
        ...
        return (
            ...
            currentTaPZfP,
            currentTaWZfW,
            int(currentTraitLevel[0]),
            int(currentTraitLevel[1]),
            int(currentTraitLevel[2]),
        )
    ...
    def initativeResult(self, firstLine: str, secondLine: str):
        ...
        currentIni = int(
            re.split(r" ", secondLine)[2].replace("\tBE:", "").replace("\tBE:\tMod.:", "").replace("\tMod.:", "")
        )
    ...
    def insert_spell_roll(self, roll):
        ...
        """,
            (
                self.get_character_id(self.currentChar),
                roll[1],
                roll[2],
                roll[3],
                roll[4],
    ...
    def main(self, chatlogLines):
        ...
        for i in range(len(chatlogLines)):
            ...
            if self.validateTrait(potentialEvent):
                currentMod, currentSuccess, currentTaPZfP, currentTaWZfW = self.traitResult(
                    chatlogLines[i + 1].strip(), chatlogLines[i + 2].strip()
                )
                roll = [
                    self.currentChar,
                    "Eigenschaftsprobe",
                    potentialEvent,
                    TRAITS[TRAITS_LONG.index(potentialEvent)],
                    currentMod,
                    currentSuccess,
                    currentTaPZfP,
                    currentTaWZfW,
            ...
            if self.validateTalent(potentialEvent):
                ...
                currentMod, currentSuccess, currentTaP, currentTaW, currentTrait1, currentTrait2, currentTrait3 = (
                    self.talentResult(chatlogLines[i + 1].strip(), chatlogLines[i + 2].strip())
                )
                roll = [
                    self.currentChar,
                    category,
                ...
                self.updateTraitValues(
                    self.currentChar, [trait1, trait2, trait3], [currentTrait1, currentTrait2, currentTrait3]
                )
                continue

            if self.validateSpell(potentialEvent):
                category, trait1, trait2, trait3 = self.getTraits("spell", potentialEvent)
                currentMod, currentSuccess, currentZfP, currentZfW, currentTrait1, currentTrait2, currentTrait3 = (
                    self.spellResult(chatlogLines[i + 1].strip(), chatlogLines[i + 2].strip())
                )
                roll = [
                    self.currentChar,
                    category,
            ...
            if self.validateAttack(potentialEvent):
                ...
                currentMod, currentSuccess, currentTaPZfP, currentTaWZfW = self.attackResult(
                    chatlogLines[i + 1].strip(), thirdLine
                )
                roll = [
                    self.currentChar,
                    "Kampfprobe",
            ...
            if (
                "Initiative" in potentialEvent
                and not "Initiativewurf" in potentialEvent
                and self.currentChar in potentialEvent
            ):
                currentIni, rolledIni, currentMod = self.initativeResult(
                    chatlogLines[i].strip(), chatlogLines[i + 1].strip()
                )
    ...
    def update_character_traits(self):
        ...
        # Update the characters table in the database
        for _, row in trait_values_df.iterrows():
            update_query = """
                UPDATE characters
                SET Mut = %s, Klugheit = %s, Intuition = %s, Charisma = %s, Fingerfertigkeit = %s, Gewandtheit = %s, Konstitution = %s, Körperkraft = %s
                WHERE name = %s
            """
            self.cursor.execute(
                update_query,
