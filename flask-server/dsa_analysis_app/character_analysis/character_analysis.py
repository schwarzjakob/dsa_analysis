...
def get_character_attacks(character_name):
    ...
    filtered_df = df[df[CHARACTER_KEY] == character_name] # Filter the DataFrame to only include the specified character
    attacks_count = filtered_df[TALENT_KEY].value_counts() # Count the occurrences of each attack
    return attacks_count.sort_values(ascending=False).to_dict() # Convert to dictionary and sort by descending order
...
def get_character_attack_statistics(character_name, attack):
    ...
    # Convert all int64 values to native Python integers
    attack_statistics = {key: int(value) if isinstance(value, np.int64) else value 
